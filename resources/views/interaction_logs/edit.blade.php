                            ...
                            <div>
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-0">
                                        <li class="breadcrumb-item"><a href="{{ route('interaction-logs.index') }}">Interaction Logs</a></li>
                                        <li class="breadcrumb-item active">Edit Log</li>
                                    </ol>
                                </nav>
                            </div>
                            <div class="badge bg-{{ 
                                $interactionLog->type == 'call' ? 'info' : 
                                ($interactionLog->type == 'email' ? 'success' : 'warning') 
                            }} text-white">
                                {{ ucfirst($interactionLog->type) }}
                            </div>
                        </div>
    
                        <!-- Edit Interaction Form -->
                        <form method="POST" action="{{ route('interaction-logs.update', $interactionLog) }}" class="needs-validation" novalidate>
                            @method('PUT')
                            @csrf
                            ...
                            <div class="row g-3">
                                    ...
                                    <div class="form-floating">
                                        <select name="client_id" class="form-select" id="clientSelect" required>
                                            @foreach($clients as $client)
                                                <option value="{{ $client->id }}" @selected(old('client_id', $interactionLog->client_id) == $client->id)>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select name="type" class="form-select" id="typeSelect" required>
                                            <option value="call" @selected(old('type', $interactionLog->type) == 'call')>
                                                Call
                                            </option>
                                            <option value="email" @selected(old('type', $interactionLog->type) == 'email')>
                                                Email
                                            </option>
                                            <option value="meeting" @selected(old('type', $interactionLog->type) == 'meeting')>
                                                Meeting
                                            </option>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        ...
                                        value="{{ old('interaction_date', \Carbon\Carbon::parse($interactionLog->interaction_date)->format('Y-m-d')) }}" required
                                        max="{{ date('Y-m-d') }}">
                                ...
                                <div class="col-12">
                                    <div class="form-floating">
                                        <textarea name="notes" class="form-control" id="notesInput" 
                                                  placeholder="Interaction details" style="height: 120px">{{ old('notes', $interactionLog->notes) }}</textarea>
                                ...
                                <div class="col-12 mt-4">
                                    <div class="d-flex justify-content-between">
                                        <a href="{{ route('interaction-logs.index') }}" class="btn btn-outline-secondary px-4">
